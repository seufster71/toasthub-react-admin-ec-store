...
export function init() {
	return function(dispatch) {
		...
		requestParams.prefTextKeys = new Array("ADMIN_EC_STORE_PAGE");
		requestParams.prefLabelKeys = new Array("ADMIN_EC_STORE_PAGE");
		let params = {};
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "LOAD_INIT_EC_STORE", responseJson });
			} else {
...
export function list({state,listStart,listLimit,searchCriteria,orderCriteria,info,paginationSegment}) {
	return function(dispatch) {
		...
		let prefChange = {"page":"ecstore","orderCriteria":requestParams.orderCriteria,"listStart":requestParams.listStart,"listLimit":requestParams.listLimit};
		dispatch({type:"EC_STORE_PREF_CHANGE", prefChange});
		let params = {};
		params.requestParams = requestParams;
		params.URI = '/api/admin/callService';
		...
		return callService(params).then( (responseJson) => {
			if (responseJson != null && responseJson.protocalError == null){
				dispatch({ type: "LOAD_LIST_EC_STORE", responseJson, paginationSegment });
				if (info != null) {
...
export function modifyItem({id, appPrefs}) {
	return function(dispatch) {
	    ...
	    requestParams.service = "EC_STORE_SVC";
	    requestParams.prefFormKeys = new Array("ADMIN_EC_STORE_FORM");
	    if (id != null) {
	    	requestParams.itemId = id;
	    }
	    let params = {};
...
export function searchChange({value}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_STORE_SEARCH_CHANGE",value});
	 };
}
...
export function setErrors({errors}) {
	 return function(dispatch) {
		 dispatch({ type:"EC_STORE_SET_ERRORS",errors});
	 };
}
